...
def page_overview(meta: pd.DataFrame, engine, f: Dict) -> None:
    ...
    def _render_opt_cards(df_target: pd.DataFrame, item_name: str):
        if df_target is None or df_target.empty:
            st.info(f"조회된 {item_name} 데이터가 없습니다.")
            return
        ...
        with c1:
            with st.container(border=True):
                ...
                st.caption(f"비용 3만 원 이상 소진 중이나 전환이 0건인 항목 (제외 권장)")
                hippos = df_target[(df_target['cost'] >= 30000) & (df_target['conv'] == 0)].sort_values('cost', ascending=False)
                if not hippos.empty:
                    disp_h = hippos[['account_name', 'keyword', 'cost']].rename(columns={'account_name': '업체명', 'keyword': item_name, 'cost': '비용'})

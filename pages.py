    ...
    if not df_pl_kw.empty:
        ...
        with c1:
            with st.container(border=True):
                ...
                st.caption("비용 3만 원 이상 소진 중이나 전환이 0건인 파워링크 키워드 (제외 권장)")
                hippos = df_pl_kw[(df_pl_kw['cost'] >= 30000) & (df_pl_kw['conv'] == 0)].sort_values('cost', ascending=False)
                if not hippos.empty:
        ...
        with c2:
            with st.container(border=True):
                ...
                st.caption("비용 3천원~5만 원 소진, ROAS 500% 이상 우수 파워링크 키워드 (입찰가 상향 권장)")
                stars = df_pl_kw[(df_pl_kw['cost'] >= 3000) & (df_pl_kw['cost'] <= 50000) & (df_pl_kw['conv'] >= 1) & (df_pl_kw['roas'] >= 500)].sort_values('roas', ascending=False)
                if not stars.empty:
        ...
        st.info("파워링크 키워드 데이터가 수집되지 않아 분석할 수 없습니다.")
...
def page_perf_keyword(meta: pd.DataFrame, engine, f: Dict):
    ...
    with tab_shop:
        st.error("🚨 네이버 공식 API 정책상 '쇼핑검색의 실제 검색어'는 자동 수집을 지원하지 않습니다.")
        st.info("💡 **대안:** 쇼핑검색의 주요 성과는 좌측 메뉴의 **[🧩 소재]** 탭에서 '상품(소재)별'로 확인하실 수 있습니다.")

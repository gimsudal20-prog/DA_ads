...
def page_budget(meta: pd.DataFrame, engine, f: Dict) -> None:
    ...
    # ✨ [핵심 조치 1] 두 개의 탭으로 분리하여 광고 꺼짐 알림 전용 탭을 만들었습니다.
    tab_budget, tab_alert = st.tabs(["💰 월 예산 및 집행 현황", "🚨 광고 중단(꺼짐) 예측일"])
    ...
    with tab_budget:
        ...
        c1, c2 = st.columns(2)
        with c1: ui_metric_or_stmetric(f"{end_dt.month}월 총 사용액", format_currency(total_month_cost), f"{end_dt.strftime('%Y-%m')} 누적", key='m_month_cost')
        with c2: ui_metric_or_stmetric('효율 ☔ 비상 계정', f"{count_rain}건", f'목표 ROAS {target_roas}% 미달', key='m_need_opt')
    ...
    with tab_alert:
        ...
        with c1: ui_metric_or_stmetric('총 비즈머니 잔액', format_currency(total_balance), '전체 합산', key='m_total_balance_alert')

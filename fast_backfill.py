# -*- coding: utf-8 -*-
"""fast_backfill.py - 과거 데이터 총합/분리 전환 재수집 및 롤링 수집 스크립트"""
...
if not args.start or not args.end:
    ...
    print("ℹ️ 날짜 인자가 없습니다. 자동으로 '최근 7일 총합+분리 전환 업데이트 모드'로 작동합니다.")
else:
    start_date = datetime.strptime(args.start, "%Y-%m-%d").date()
    end_date = datetime.strptime(args.end, "%Y-%m-%d").date()

print(f"🚀 [총합 + 분리 전환 수집] 백필 작업을 시작합니다: {start_date} ~ {end_date}")
...
while curr_date <= end_date:
    ...
    print(f"\n" + "="*60)
    print(f"📅 [ {d_str} ] 데이터 수집 및 전환 분리 성과 매핑 진행 ⚡")
    print("="*60)
    
    # 1. 네이버 검색광고(SA) 수집 실행
    # (collector.py가 총합 지표와 purchase/cart 분리 지표를 함께 적재하도록 업데이트됨)
    ...
    try:
        # 수집기 실행 (이 과정에서 총합/분리 전환 적재가 자동으로 일어남)
...
print("\n" + "★"*30)
print("🎉 지정된 기간의 총합/분리 전환 데이터 백필이 모두 완료되었습니다!")
print("★" * 30)

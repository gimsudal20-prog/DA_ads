...
def _apply_comparison_metrics(view_df: pd.DataFrame, base_df: pd.DataFrame, merge_keys: list) -> pd.DataFrame:
    ...
    for c in ['imp', 'clk', 'cost', 'cart_conv', 'cart_sales', 'conv', 'sales']:
        if c in base_df.columns:
            base_df[c] = pd.to_numeric(base_df[c], errors='coerce').fillna(0)
            
    agg_dict = {'imp': 'sum', 'clk': 'sum', 'cost': 'sum', 'cart_conv': 'sum', 'cart_sales': 'sum', 'conv': 'sum', 'sales': 'sum'}
    ...
    if not base_df.empty:
        ...
        base_agg = base_agg.rename(columns={'imp': 'b_imp', 'clk': 'b_clk', 'cost': 'b_cost', 'cart_conv': 'b_cart_conv', 'cart_sales': 'b_cart_sales', 'conv': 'b_conv', 'sales': 'b_sales', 'avg_rank': 'b_avg_rank'})
        merged = pd.merge(view_df, base_agg, on=merge_keys, how='left')
    else:
        merged = view_df.copy()
        
    for c in ['b_imp', 'b_clk', 'b_cost', 'b_cart_conv', 'b_cart_sales', 'b_conv', 'b_sales']:
    ...
    merged['이전 ROAS(%)'] = np.where(merged['이전 광고비'] > 0, (merged['이전 구매완료 매출'] / merged['이전 광고비']) * 100, 0.0)
    merged['ROAS 증감(%)'] = merged['구매 ROAS(%)'] - merged['이전 ROAS(%)']
...
def render_keyword_main(view, top_n, fmt):
    ...
    metrics_cols = ["노출", "클릭", "CTR(%)", "CPC(원)", "광고비", "장바구니수", "장바구니 ROAS(%)", "구매완료수", "CPA(원)", "구매완료 매출", "구매 ROAS(%)"]
...
def render_keyword_cmp(view, engine, cids, type_sel, top_n, fmt_cmp, start_dt, end_dt):
    ...
    if not depth_toggle_kw:
        ...
        disp["장바구니ROAS 증감 "] = disp["장바구니ROAS 증감"].apply(lambda x: f"{x:+.1f}%" if pd.notna(x) and x != 0 else "-")
        ...
        metrics_cols_cmp = ["노출", "노출 증감/율", "클릭", "클릭 증감/율", "광고비", "광고비 증감/율", "CPC(원)", "CPC 증감/율", "장바구니수", "장바구니 증감/율", "장바구니 ROAS(%)", "장바구니ROAS 증감 ", "구매완료수", "구매 증감 ", "구매 ROAS(%)", "ROAS 증감 "]
        delta_cols = ["노출 증감/율", "클릭 증감/율", "광고비 증감/율", "CPC 증감/율", "장바구니 증감/율", "장바구니ROAS 증감 ", "구매 증감 ", "ROAS 증감 "]
    else:
        metrics_cols_cmp = [
            ...
            "이전 장바구니 ROAS(%)", "장바구니 ROAS(%)", "장바구니ROAS 증감",
            "이전 구매완료수", "구매완료수", "구매 증감", 
            "이전 ROAS(%)", "구매 ROAS(%)", "ROAS 증감(%)"
        ]
        delta_cols = ["노출 증감(%)", "노출 증감", "클릭 증감(%)", "클릭 증감", "광고비 증감(%)", "광고비 증감", "CPC 증감(%)", "CPC 증감", "순위 변화", "장바구니 증감(%)", "장바구니 증감", "장바구니ROAS 증감", "구매 증감", "ROAS 증감(%)"]
...
def page_perf_keyword(meta: pd.DataFrame, engine, f: Dict) -> None:
    ...
    fmt = {
        ...
        "CPA(원)": "{:,.0f}", "구매완료 매출": "{:,.0f}", "구매완료수": "{:,.1f}", "CTR(%)": "{:,.2f}%", "구매 ROAS(%)": "{:,.2f}%"
    }
    ...
    with tab_cmp:
        ...
        fmt_cmp.update({
            ...
            "이전 장바구니 ROAS(%)": "{:,.2f}%", "장바구니ROAS 증감": "{:+.2f}%",
            "이전 구매완료수": "{:,.1f}", "구매 증감": "{:+.1f}",

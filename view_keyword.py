...
def compute_keyword_view(kw_bundle, ad_bundle, meta):
    ...
    # 노출용 상품명(키워드) 열을 완전히 삭제하고 '광고그룹' 단위로 모두 합산 처리
    grp_cols = ["customer_id", "adgroup_id", "업체명", "담당자", "캠페인유형", "캠페인", "광고그룹"]
    agg_dict = {"노출": "sum", "클릭": "sum", "광고비": "sum", "전환": "sum", "전환매출": "sum"}
    if "avg_rank" in view.columns:
        agg_dict["avg_rank"] = "mean"
        
    view = view.groupby([c for c in grp_cols if c in view.columns], as_index=False).agg({k: v for k, v in agg_dict.items() if k in view.columns})
...
def render_keyword_main(view, top_n, fmt):
    if view.empty:
        st.info("해당 기간의 상세 성과 데이터가 없습니다.")
        return
    ...
    st.markdown("<div style='font-size:14px; font-weight:700; margin-bottom:12px; margin-top:20px;'>상세 종합 성과 데이터</div>", unsafe_allow_html=True)
...
def render_keyword_cmp(view, engine, cids, type_sel, top_n, fmt_cmp, start_dt, end_dt):
    ...
    if view.empty:
        st.info("현재 기간의 데이터가 없습니다.")
        return
        
    base_bundle = pd.concat([base_kw_bundle, base_ad_bundle], ignore_index=True)

    view_cmp = view.copy()
    if not base_bundle.empty:
        valid_keys = [k for k in ["customer_id", "adgroup_id"] if k in view_cmp.columns and k in base_bundle.columns]
        if valid_keys:
    ...
    base_cols_cmp = ["업체명", "담당자", "캠페인유형", "캠페인", "광고그룹"]
    if "avg_rank" in view_cmp.columns or "평균순위" in view_cmp.columns:
        base_cols_cmp.extend(["평균순위", "이전 평균순위", "순위 변화"])

    # 검색 기준을 광고그룹으로 변경
    base_bundle_renamed = base_bundle.rename(columns={"adgroup_name": "광고그룹"}) if "adgroup_name" in base_bundle.columns else base_bundle
    render_item_comparison_search("광고그룹", view_cmp, base_bundle_renamed, "광고그룹", start_dt, end_dt, b1, b2)
    ...
    st.markdown("<div style='font-size:14px; font-weight:700; margin-bottom:12px; margin-top:8px;'>상세 기간 비교 표</div>", unsafe_allow_html=True)
...
def page_perf_keyword(meta: pd.DataFrame, engine, f: Dict) -> None:
    if not f.get("ready", False):
        return
    st.markdown("<div class='nv-sec-title'>광고그룹 상세 분석</div>", unsafe_allow_html=True)
    st.caption("캠페인 하위 단위인 '광고그룹'별 성과를 보여줍니다.")

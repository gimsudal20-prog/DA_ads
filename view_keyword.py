...
def _apply_comparison_metrics(view_df: pd.DataFrame, base_df: pd.DataFrame, merge_keys: list) -> pd.DataFrame:
    ...
    # 통합 증감 (과거 호환)
    merged['이전 총 전환수'] = merged['이전 구매완료수'] + merged['이전 장바구니수']
    merged['이전 총 전환매출'] = merged['이전 구매완료 매출'] + merged['이전 장바구니 매출액']
    ...
    with tab_main:
        if view.empty:
            ...
            if not funnel_toggle:
                ...
                metrics_cols = ["노출", "클릭", "CTR(%)", "CPC(원)", "광고비", "장바구니수", "장바구니 ROAS(%)", "구매완료수", "구매완료 매출", "구매 ROAS(%)"]
    ...
    with tab_cmp:
        ...
        if view.empty:
            ...
            if not funnel_toggle:
                ...
                metrics_cols_cmp = ["노출", "노출 증감/율", "클릭", "클릭 증감/율", "광고비", "광고비 증감/율", "CPC(원)", "CPC 증감/율", "총 전환수", "총 전환 증감 ", "통합 ROAS(%)", "통합 ROAS 증감 "]
                ...
                delta_cols = ["노출 증감(%)", "노출 증감", "클릭 증감(%)", "클릭 증감", "광고비 증감(%)", "광고비 증감", "장바구니 증감(%)", "장바구니 증감", "장바구니ROAS 증감", "구매 증감", "구매 ROAS 증감(%)"]

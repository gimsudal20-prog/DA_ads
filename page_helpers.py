...
def append_comparison_data(df_cur: pd.DataFrame, df_prev: pd.DataFrame, join_keys: list) -> pd.DataFrame:
    ...
    out["ROAS 증감"] = out[roas_delta_col].apply(lambda x: "-" if pd.isna(x) or x==0 else (f"▲ {x:.2f}%p" if x > 0 else f"▼ {abs(x):.2f}%p"))
...
def render_period_compare_badges(d1: date, d2: date, b1: date, b2: date, c_cur: dict, c_base: dict):
    def badge(label, key):
        cur = float(c_cur.get(key, 0) or 0)
        base = float(c_base.get(key, 0) or 0)
        ch = pct_change(cur, base)
        arrow = pct_to_arrow(ch)
        ch_txt = "-" if ch is None else f"{ch:+.1f}%"
        return f"""
        <div style='padding:12px 14px; border:1px solid #e5e7eb; border-radius:10px; background:#fff;'>
          <div style='font-size:12px; color:#6b7280;'>{label}</div>
          <div style='font-size:20px; font-weight:800; margin-top:2px;'>{int(cur):,}</div>
          <div style='font-size:12px; margin-top:2px; color:#111827;'>비교 {int(base):,} · <b>{arrow} {ch_txt}</b></div>
        </div>
        """
    st.markdown(
        f"""
        <div style='margin:8px 0 10px; color:#6b7280; font-size:12px;'>
          선택기간: <b>{d1} ~ {d2}</b> · 비교기간: <b>{b1} ~ {b2}</b>
        </div>

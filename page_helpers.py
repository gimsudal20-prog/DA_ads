...
def build_filters(meta: pd.DataFrame, type_opts: List[str], engine=None) -> Dict:
    ...
    with st.expander("🔍 조회 기간 및 필터 설정", expanded=st.session_state.get("filters_expanded", True)):
        ...
        with basic_filter_container:
            ...
            available_accounts = meta.copy()
            if manager_sel and "manager" in available_accounts.columns:
                available_accounts = available_accounts[available_accounts["manager"].isin(manager_sel)]
            account_options = sorted([x for x in available_accounts["account_name"].dropna().unique().tolist() if str(x).strip()]) if "account_name" in available_accounts.columns else []
            account_sel = ui_multiselect(st, "업체 필터", account_options, default=sv.get("account", []), key="f_account", placeholder="모든 업체")
            type_sel = ui_multiselect(st, "캠페인 유형", type_opts, default=sv.get("type_sel", []), key="f_type", placeholder="전체 유형")

        with st.expander("⚙️ 고급 옵션", expanded=False):
            adv_col1, adv_col2 = st.columns(2)
            top_n_keyword = adv_col1.number_input("키워드 상위 N", min_value=50, max_value=5000, value=int(sv.get("top_n_keyword", 300)), step=50, key="f_topn_kw")
            top_n_ad = adv_col2.number_input("소재 상위 N", min_value=50, max_value=3000, value=int(sv.get("top_n_ad", 200)), step=50, key="f_topn_ad")
            prefetch_warm = st.checkbox("초기 진입 시 데이터 미리 불러오기", value=bool(sv.get("prefetch_warm", True)), key="f_prefetch")

        c_apply, c_reset = st.columns([1, 1])
        with c_apply:
            applied = st.button("적용", type="primary", use_container_width=True)
        with c_reset:
    ...
    cur = st.session_state["filters_v8"]
    selected_customer_ids = resolve_customer_ids(meta, cur.get("manager", []), cur.get("account", []))
    cmp_opts = get_dynamic_cmp_options(cur["d1"], cur["d2"])
    cmp_mode = st.selectbox("비교 방식", cmp_opts, index=0, key="f_cmp_mode")

    b1, b2 = (None, None)
    if cmp_mode != "비교 안함":
        b1, b2 = period_compare_range(cur["d1"], cur["d2"], cmp_mode)

    return {
        "q": cur.get("q", ""),
        "manager": cur.get("manager", []),
        "account": cur.get("account", []),
        "type_sel": tuple(cur.get("type_sel", [])),
        "start": cur["d1"],
        "end": cur["d2"],
        "compare_mode": cmp_mode,
        "base_start": b1,
        "base_end": b2,
        "top_n_keyword": int(cur.get("top_n_keyword", 300)),
        "top_n_ad": int(cur.get("top_n_ad", 200)),
        "top_n_campaign": int(cur.get("top_n_campaign", 200)),
        "prefetch_warm": bool(cur.get("prefetch_warm", True)),
...
def _perf_common_merge_meta(df: pd.DataFrame, meta: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    if meta is None or meta.empty or "customer_id" not in out.columns or "customer_id" not in meta.columns:
        return out
    m = meta.copy()
    m["customer_id"] = pd.to_numeric(m["customer_id"], errors="coerce").astype("Int64").astype(str)
    out["customer_id"] = pd.to_numeric(out["customer_id"], errors="coerce").astype("Int64").astype(str)
    cols = [c for c in ["customer_id", "account_name", "manager"] if c in m.columns]
    return out.merge(m[cols].drop_duplicates(), on="customer_id", how="left")
...
def render_item_comparison_search(entity_label: str, df_cur: pd.DataFrame, df_base: pd.DataFrame, name_col: str, d1: date, d2: date, b1: date, b2: date):
    ...
    rows = [
        {"label": "광고비", "base": fmt_krw(b_cost), "curr": fmt_krw(c_cost), "delta": calc_detail_delta(c_cost, b_cost, is_currency=True), "rate": calc_delta_rate(c_cost, b_cost)},
        {"label": "전환매출", "base": fmt_krw(b_sales), "curr": fmt_krw(c_sales), "delta": calc_detail_delta(c_sales, b_sales, is_currency=True), "rate": calc_delta_rate(c_sales, b_sales)},
        {"label": "ROAS", "base": fmt_pct(b_roas), "curr": f"<span style='font-weight:900; color:#dc2626;'>{fmt_pct(c_roas)}</span>", "delta": calc_detail_delta(c_roas, b_roas, is_pct=True), "rate": calc_delta_rate(c_roas, b_roas)},
        {"label": "노출수", "base": fmt_num(b_imp), "curr": fmt_num(c_imp), "delta": calc_detail_delta(c_imp, b_imp), "rate": calc_delta_rate(c_imp, b_imp)},
        {"label": "클릭수", "base": fmt_num(b_clk), "curr": fmt_num(c_clk), "delta": calc_detail_delta(c_clk, b_clk), "rate": calc_delta_rate(c_clk, b_clk)},
        {"label": "전환수", "base": fmt_num(b_conv), "curr": fmt_num(c_conv), "delta": calc_detail_delta(c_conv, b_conv), "rate": calc_delta_rate(c_conv, b_conv)},
    ]

...
def page_perf_keyword(meta: pd.DataFrame, engine, f: Dict) -> None:
    ...
    top_n = int(f.get("top_n_keyword", 100))
    cids = tuple(f.get("selected_customer_ids", []) or [])
    type_sel = tuple(f.get("type_sel", tuple()) or tuple())

    df = query_keyword_topn(engine, f["start"], f["end"], cids, type_sel, top_n)
    if df is None or df.empty:
        st.warning("데이터 없음 (dim_keyword/dim_adgroup/dim_campaign 또는 fact_keyword_daily 확인)")
        return
    ...
    st.dataframe(view_df, use_container_width=True, hide_index=True)
    render_download_compact(view_df, f"성과_키워드_TOP{top_n}_{f['start']}_{f['end']}", "keyword", "kw")

...
def color_delta_positive(val):
    if pd.isna(val) or val == 0: return 'color: var(--nv-muted-light);'
    return 'color: var(--nv-primary); font-weight: 600;' if val > 0 else 'color: var(--nv-danger); font-weight: 600;'

def color_delta_negative(val):
    if pd.isna(val) or val == 0: return 'color: var(--nv-muted-light);'
    return 'color: var(--nv-danger); font-weight: 600;' if val > 0 else 'color: var(--nv-primary); font-weight: 600;'
...
def page_overview(meta: pd.DataFrame, engine, f: Dict) -> None:
    ...
    # ==========================================
    # 1. 핵심 성과 지표 (KPI Cards)
    # ==========================================
    st.markdown(f"<div class='nv-sec-title'>{account_name} 요약 ({selected_type_label})</div>", unsafe_allow_html=True)
    
    cmp_date_info = f"{cmp_mode} ({b1} ~ {b2})" if b1 and b2 else cmp_mode
    st.markdown(f"<div style='font-size:13px; color:var(--nv-muted); margin-bottom:20px;'>비교 기준: <span style='font-weight:600;'>{cmp_date_info}</span></div>", unsafe_allow_html=True)
    ...
    # ==========================================
    # 3. 상세 데이터 탐색 (표 데이터를 탭으로 숨겨 미니멀리즘 구현)
    # ==========================================
    ...
    with tab_acc:
        ...
        if not cur_camp.empty or not base_camp.empty:
            ...
            st.download_button("CSV 다운로드", data=csv_account_data, file_name="업체별_성과_요약.csv", mime="text/csv", key="btn_acc_csv")
        else:
            st.info("데이터가 없습니다.")
    ...
    with tab_type:
        ...
        if type_col and not cur_camp.empty:
            cur_type_grp = cur_camp.groupby(type_col)[['imp', 'clk', 'cost', 'conv', 'sales']].sum().reset_index()
    ...
    # ------------------ 탭 3. 기간별 상세 (주간/일간) ------------------
    with tab_period:
        sub_t1, sub_t2 = st.tabs(["주간 요약", "일자별 데이터"])
        with sub_t1:
            ...
            if base_weekly_ts is not None and not base_weekly_ts.empty:
                def _get_week_info(dt_val):
                    ...
                    return f"{start.strftime('%m/%d')} ~ {(start + timedelta(days=6)).strftime('%m/%d')}", start
                ...
                st.dataframe(weekly_disp.style.format({
    ...
    with st.expander("📝 빠른 텍스트 보고서 복사", expanded=False):
        report_campaign_type = selected_type_label
        report_cur = cur_summary
